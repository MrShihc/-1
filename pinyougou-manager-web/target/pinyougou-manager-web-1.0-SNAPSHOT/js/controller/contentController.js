...
app.controller('contentController' ,function($scope,$controller,contentService,uploadService,contentCategoryService){
	...
	$scope.findPage=function(searchEntity,page,rows){
		contentService.search(page,rows,searchEntity).success(
			function(response){
	...
	//批量开启广告
	$scope.startContent = function(status){
		var ids = $scope.selectIds;
		// alert($scope.selectIds);
		if(confirm("你确认要开启吗？")){
			contentService.startContent(status,ids).success(function(response){
				if(response.success){
					$scope.reloadList();
					$scope.selectIds = [];
				}else{
